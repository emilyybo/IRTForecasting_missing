...
model{
    ...
    ## of missingness:
    x[i,j] <- exp((1-(1-r[j])^2)*(lambda[ifpidx[i],1]*(theta[uidx[i],1] - b0[ifpidx[i]] - rho[j])))
    }
...
inits <- list(b0 = rep(0, data$M), theta = matrix(0, data$N, 2), rho=rep(0,6))

## smaller number of iterations, to see how things are working
library(runjags)
mfit <- run.jags("missmod.jag", data=data, inits=inits, n.chains=3, monitor=c("theta","b0","lambda","rho", "g0"), burnin=5000, sample=20000, method="parallel")
#mfit$summary$statistics[100:250,]

#postmeans <- summary(mfit)$statistics[,1]
#rho <- postmeans[grep("rho", names(postmeans))] 

save(mfit, file="IRTforecasting_missingwithRho_v2.rda")

#load("IRTforecasting_missingwithRho.rda")
#psrf_v1 <- gelman.diag(mfit, multivariate=FALSE)
